...
COLS = 90
CLAHE_CLIP = 3.0
GAMMA = 1.7          # the darkening curve fix: (v/255)^1.7
CROP_BOTTOM = 0.0    # trim this fraction off the bottom, e.g. 0.15 to cut torso/chair
...
def prep(path, already_cut=False):
    src = Image.open(path).convert("RGBA")
    cut = src if already_cut else remove(src)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("usage: python make_ascii_svg.py photo.jpg [out.svg]")
        sys.exit(1)
    src = sys.argv[1]
    dst = sys.argv[2] if len(sys.argv) > 2 else "ascii.svg"
    lines = to_lines(prep(src, already_cut=True))
    print("\n".join(lines))
    build_svg(lines, dst)
